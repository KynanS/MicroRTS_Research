...
public class UnitTypeTable  {
    ...
    // NEW:
    public UnitType killer = getNewUnit();
    ...
    public void setUnitTypeTable(int version, int crs) {       
        ...
        UnitType factory = getFactory(version);
        addUnitType(factory);
        // WORKER: 
        ...
        barracks.produces(killer);  
        barracks.produces(light);
        barracks.produces(heavy);
        barracks.produces(ranged);
        factory.produces(light);
        factory.produces(heavy); 
        factory.produces(ranged);
        worker.produces(base);
        worker.produces(barracks);
        worker.produces(factory);
    }
    ...
    public UnitType getFactory(int version){
        
        UnitType factory = new UnitType();
        factory.name = "Factory";
        factory.cost = 5;
        factory.hp = 4;
        switch(version) {
            case VERSION_ORIGINAL: 
                factory.produceTime = 200;
                break;
            case VERSION_ORIGINAL_FINETUNED: 
            case VERSION_NON_DETERMINISTIC:
                factory.produceTime = 100;
                break;
        }
        factory.isResource = false;
        factory.isStockpile = false;
        factory.canHarvest = false;
        factory.canMove = false;
        factory.canAttack = false;
        factory.sightRadius = 3;
        return(factory);
    }
