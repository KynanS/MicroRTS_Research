...
public class UnitTypeTable  {
    ...
    public void setUnitTypeTable(int version, int crs) {       
        ...
        // RESOURCE:
        UnitType resource = getResource();
        addUnitType(resource);           
                
        // BASE:
        UnitType base = getBase(version);
        addUnitType(base);

        // BARRACKS: 
        UnitType barracks = getBarracks(version);
        addUnitType(barracks);
        
        // WORKER: 
        UnitType worker = getWorker(version);
        addUnitType(worker);   
        
        // LIGHT: 
        UnitType light = getLight(version);
        addUnitType(light);           

        // HEAVY: 
        UnitType heavy = getHeavy(version);
        addUnitType(heavy);           

        // RANGED: 
        UnitType ranged = getRanged(version);
        addUnitType(ranged);     

        // RANDOM:
        UnitType randomUnit = getRandomUnit(version);
        addUnitType(randomUnit);
        ...
        barracks.produces(randomUnit);
        worker.produces(base);
        worker.produces(barracks);
    }

    public UnitType getResource(){
    ...
    public UnitType getBase(int version){
    ...
    public UnitType getBarracks(int version){
    ...
    public UnitType getWorker(int version){
    ...
    public UnitType getLight(int version){
    ...
    public UnitType getRandomUnit(int version){

        UnitType randomUnit = new UnitType();
        Random rnd = new Random();
        randomUnit.name = "Random";
        randomUnit.cost = 1 + rnd.nextInt(2);
        randomUnit.hp = 1 + rnd.nextInt(4);
        randomUnit.minDamage = 1;
        randomUnit.maxDamage = 1 + rnd.nextInt(2);
        randomUnit.attackRange = 1 + rnd.nextInt(3);
        randomUnit.produceTime = 100 + rnd.nextInt(21);
        randomUnit.moveTime = 8 + rnd.nextInt(5);
        randomUnit.attackTime = 5;
        randomUnit.isResource = false;
        randomUnit.isStockpile = false;        
        randomUnit.canHarvest = false;
        randomUnit.canMove = true;
        randomUnit.canAttack = true;
        randomUnit.sightRadius = 3 + rnd.nextInt(3);
